...
function conct(astring, times = 1) {
    var result = ""
...
function finalExam(year, gender) {
    var age = whichGender(gender);
    var time = timeLeft(year, age);

    if (time < 0) {
        return "CESTITAMO! U PENZIJI STE!"
    } else {
        if (isNaN(time)) {
            return "Unesite M ili F u gender";
        } else {
            return "Do penzije ima jos: " + time + " godina";
        }
    } 
}

console.log(finalExam(1977, "Q"));